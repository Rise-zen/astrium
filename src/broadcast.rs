...
/// between `# >>> refract` / `# <<< refract` markers (creating it on first
...
fn replace_block(existing: &str, block: &str) -> String {
    const START: &str = "# >>> refract";
    const END: &str = "# <<< refract";
...
fn notify_nvim_instances(cache_dir: &Path) {
    ...
    for entry in entries.flatten() {
        ...
        let ok = Command::new("nvim")
            .args(["--server", &sock.to_string_lossy(), "--remote-expr", "v:lua.RefractReload()"])
            .output()
...
fn write_and_apply_hyprland(colors: &Colors, cache_dir: &Path) -> Result<()> {
    ...
    for (key, value) in commands {
        if let Err(e) = Command::new("hyprctl")
            ...
            .status()
        {
            eprintln!("[refract] hyprctl {key}: {e}");
        }
    }
...
fn write_nvim_theme(colors: &Colors, cache_dir: &Path) -> Result<()> {
    ...
    writeln!(f, "-- Auto-generated by refract, do not edit by hand")?;
    writeln!(f, "local M = {{}}")?;
    writeln!(f)?;
